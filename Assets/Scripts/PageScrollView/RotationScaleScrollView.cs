...
public class RotationScaleScrollView : ScalePageScrollView
{
    ...
    public void ListenerItemRotation()
    {
        ...
        {
            items[lastPage].transform.localRotation = Quaternion.Euler(-Vector3.Slerp(Vector3.zero, new Vector3(0, rotation, 0), percent));
            items[nextPage].transform.localRotation = Quaternion.Euler(Vector3.Slerp(Vector3.zero, new Vector3(0, rotation, 0), 1 - percent));
        }
        else
        {
            items[lastPage].transform.localRotation = Quaternion.Euler(Vector3.Slerp(Vector3.zero, new Vector3(0, rotation, 0), percent));
